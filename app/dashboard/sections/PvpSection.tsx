...
export default function PvpSection({ username }: { username: string }) {
  ...
  const [p1State, setP1State] = useState<PvpPlayerState>({ data: null, loading: false, error: null });
  const [p2State, setP2State] = useState<PvpPlayerState>({ data: null, loading: false, error: null });
  ...
  const startBattle = async (e: React.FormEvent) => {
    ...
    // Fetch Player 1
    const fetch1 = fetch(`/api/github/pvp-profile?username=${encodeURIComponent(player1Name)}`)
      .then((res) => res.json())
      .then((json) => {
    ...
    // Fetch Player 2
    const fetch2 = fetch(`/api/github/pvp-profile?username=${encodeURIComponent(player2Name)}`)
      .then((res) => res.json())
      .then((json) => {
        if (json.error) throw new Error(json.error);
  ...
  return (
    <div className="flex flex-col w-full text-zinc-300">
      ...
      <div className="flex flex-col items-center justify-center text-center pt-4 pb-8 border-b border-white/5 mb-8">
        ...
        <form onSubmit={startBattle} className="mt-8 flex flex-col items-center justify-center w-full max-w-md gap-4">
          <div className="flex flex-col sm:flex-row items-center justify-center w-full gap-3">
            ...
            />
            <div className="text-zinc-600 font-bold text-xs py-1 sm:py-0">VS</div>
            <input
              type="text"
              value={player2Name}
        ...
        <div className="relative">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
            ...
            <div className="hidden md:flex absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 rounded-full bg-black border-4 border-zinc-900 items-center justify-center z-20 shadow-2xl">
               <span className="text-emerald-500 font-black italic text-xl tracking-tighter">VS</span>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── Conclusion / Verdict ── */}
      <AnimatePresence>
        {aiLoading && p1State.data && p2State.data && !aiVerdict && !aiError && (
          <motion.div
          ...
          >
            ...
            <p className="text-sm font-mono tracking-widest uppercase animate-pulse">Analyzing metrics...</p>
          </motion.div>
        )}
          ...
          <div className="mt-12 flex flex-col gap-6">
            ...
            >
              <div className="grid grid-cols-1 divide-y divide-white/5 py-2">
                {Object.entries(aiVerdict.categoryScores || aiVerdict.categoryWinners || {}).map(([cat, info]: any, idx) => {
                  const p1Score = typeof info.player1Score === 'number' ? info.player1Score : 0;
                  const p2Score = typeof info.player2Score === 'number' ? info.player2Score : 0;

                  return (
                    <div key={idx} className="flex items-center py-4 px-2 sm:px-6 gap-2 sm:gap-4 group hover:bg-white/[0.02] transition-colors">
                      {/* Player 1 Score */}
                      <div className="flex-1 flex flex-col sm:flex-row items-end sm:items-center justify-end gap-2 sm:gap-3">
                        <span className="text-sm font-bold text-white">{p1Score}<span className="text-zinc-600 font-normal">/10</span></span>
                        <div className="h-1.5 w-16 sm:w-24 bg-zinc-800 rounded-full overflow-hidden flex justify-end">
                          <div className={`h-full bg-emerald-500 ${p1Score < p2Score ? 'opacity-30' : ''}`} style={{ width: `${(p1Score / 10) * 100}%` }} />
                        </div>
                      </div>
                      
                      {/* Category Name */}
                      <div className="w-24 sm:w-32 text-center shrink-0">
                        <div className="text-[10px] sm:text-xs font-black text-zinc-500 uppercase tracking-widest">{cat.replace(/_/g, ' ')}</div>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col-reverse sm:flex-row items-start sm:items-center justify-start gap-2 sm:gap-3">
                        <div className="h-1.5 w-16 sm:w-24 bg-zinc-800 rounded-full overflow-hidden">
                          <div className={`h-full bg-emerald-500 ${p2Score < p1Score ? 'opacity-30' : ''}`} style={{ width: `${(p2Score / 10) * 100}%` }} />
                        </div>
                        <span className="text-sm font-bold text-white">{p2Score}<span className="text-zinc-600 font-normal">/10</span></span>
                      </div>
                    </div>
                  );
                })}
            ...
            >
              ...
              <div className="bg-emerald-500/5 p-8 md:py-12 flex flex-col items-center justify-center text-center relative overflow-hidden border-b border-white/5">
                ...
                <h3 className="text-xs uppercase tracking-widest text-emerald-500 font-bold mb-8 relative z-10">Match Result</h3>

                <div className="flex items-center justify-center gap-8 w-full max-w-sm mx-auto relative z-10">
                   <div className="flex flex-col items-center gap-2 flex-1">
                      <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold truncate w-full">{aiVerdict.player1}</span>
                      <span className={`text-6xl tracking-tighter font-black ${aiVerdict.score?.player1 > aiVerdict.score?.player2 ? 'text-emerald-400 drop-shadow-[0_0_15px_rgba(52,211,153,0.3)]' : 'text-zinc-400 opacity-80'}`}>{aiVerdict.score?.player1 || 0}</span>
                   </div>
                   
                   <div className="text-emerald-500/20 font-black italic text-2xl">VS</div>
                   
                   <div className="flex flex-col items-center gap-2 flex-1">
                      <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold truncate w-full">{aiVerdict.player2}</span>
                      <span className={`text-6xl tracking-tighter font-black ${aiVerdict.score?.player2 > aiVerdict.score?.player1 ? 'text-emerald-400 drop-shadow-[0_0_15px_rgba(52,211,153,0.3)]' : 'text-zinc-400 opacity-80'}`}>{aiVerdict.score?.player2 || 0}</span>
                   </div>
                </div>
              </div>
              ...
              <div className="p-8 md:p-12 text-center max-w-4xl mx-auto flex flex-col items-center justify-center">
                <h3 className="text-[20px] uppercase tracking-widest text-zinc-500 font-bold mb-6">Final Verdict</h3>
...
function PlayerCard({ state, isP1, aiVerdict }: { state: PvpPlayerState; isP1: boolean; aiVerdict: any }) {
  if (state.loading) {
    return (
      <div className={`p-6 rounded-2xl border border-white/5 bg-zinc-900/20 flex flex-col items-center justify-center min-h-[300px] ${isP1 ? "border-l-indigo-500/20" : "border-r-purple-500/20"}`}>
  ...
  const p = state.data;
  const isWinner = aiVerdict && aiVerdict.overallWinner === (isP1 ? "player1" : "player2");
  ...
  return (
    <motion.div
      ...
      className={`relative p-6 rounded-2xl border transition-colors duration-500 overflow-hidden ${
        isWinner ? "border-emerald-500/40 bg-zinc-900/60 shadow-[0_0_30px_rgba(16,185,129,0.1)]" :
        isLoser ? "border-white/5 bg-zinc-900/20 opacity-60 grayscale-[10%]" : "border-white/5 bg-zinc-900/40 hover:bg-zinc-900/60"
      }`}
    >
      {/* Background gradient hint */}
      <div className={`absolute top-0 w-full h-1/2 opacity-10 blur-3xl pointer-events-none`} />

      {isWinner && (
        <div className="absolute top-4 right-4 z-10">
           <span className="px-2 py-1 rounded-sm bg-emerald-500/20 text-emerald-400 text-[10px] font-black uppercase tracking-widest border border-emerald-500/30">Winner</span>
        </div>
      )}
      
      {isLoser && (
        <div className="absolute top-4 right-4 rotate-12 z-10">
           <span className="px-2 py-1 rounded-sm bg-zinc-800/80 text-zinc-400 text-[10px] font-bold uppercase tracking-widest border border-zinc-700">Runner-up</span>
        </div>
      )}

      <div className={`flex flex-col items-center text-center ${isP1 ? "sm:items-start sm:text-left" : "sm:items-end sm:text-right"}`}>
        <img src={p.avatar} alt={p.username} className="w-20 h-20 rounded-full mb-4 border-2 border-white/10" />
        <h3 className="text-xl font-bold text-white mb-1">{p.name || p.username}</h3>
        <a href={p.profileUrl} target="_blank" className="text-sm font-mono text-zinc-500 hover:text-white transition-colors mb-6">@{p.username}</a>
      </div>

      <div className="grid grid-cols-2 gap-2 mt-4 relative z-10">
         <StatBox label="Streak" value={p.contributions.longestStreak} />
         <StatBox label="Commits" value={p.contributions.totalCommits.toLocaleString()} />
      ...
      <div className="mt-4 pt-4 border-t border-white/5">
        <div className="flex justify-between items-center text-xs">
          <span className="text-zinc-500 uppercase tracking-wider font-bold">Lvl {Math.floor(p.accountAgeYears * 10)}</span>
          <span className="text-zinc-600">{p.uniqueLanguages} langs • {p.repoCount} repos</span>
        </div>
      </div>
    </motion.div>
  );
...
function StatBox({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="bg-black/30 border border-white/5 rounded-lg p-3 flex flex-col gap-1 items-start group hover:border-white/10 transition-colors">
      ...
      <div className="text-emerald-400 font-mono text-base font-medium">{value}</div>
    </div>
  );
}
